...
検索文と正解を固定した回帰評価セットを使い、検索ロジックの変更で既存の検索が悪化していないか確認する。
公開リポジトリには評価スクリプト・データ形式・架空のテストだけを置き、個人の検索文・正解パス・実測結果は別の非公開リポジトリで管理する。
ユーザー名を含む絶対パスはローカル設定に分離し、本文を含む索引・API送信用抜粋・認証情報はGit管理しない。
...
質問と正解は固定し、結果は実行ごとに保存する。コードのコミット、入力・索引のハッシュ、対象文書と測定条件を記録する。
実利用の失敗例は追加候補として収集し、調整に使わない評価用セットも別に持つ。探索用の質問に対する結果を未見の精度と呼ばない。
段階検索の再測定には実際のworkerのsearch→refine→refine経路を使い、下記の独立した20/80件API比較とは区別する。
